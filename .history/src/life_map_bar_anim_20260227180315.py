...
UPDATE_CACHE = True # True: 最新までチェックして不足分取得 / False: 既存CSVのみ使用
MAX_BACK = 6
START_YEAR = 2000
END_YEAR = 2021
...
def render_frame(df_year, df_prev, year, out_png, alpha=5, top_n=40) -> None:
    ...
    # =========================
    # World Average（基準線）  ※add_vlineは使わない
    # =========================
    ...
    fig.add_shape(
        ...
        y0=0, y1=1,
        xref="x2",          # 右側（col=2）のx軸
        yref="y2 domain",   # 右側（col=2）のy領域(0..1)
        line=dict(color="rgba(20,20,20,0.70)", width=2, dash="dash"),
        layer="above",
    )

    fig.add_annotation(
        xref="x2", yref="y2 domain",
        x=world_avg, y=1.02,           # 右側プロットの少し上
        text=f"World avg {world_avg:.1f}",
        showarrow=False,
        xanchor="left",
        font=dict(size=14, color="rgba(20,20,20,0.85)"),
...
def main_race(update_cache: bool = UPDATE_CACHE) -> None:
    ...
    if update_cache:
        ...
        latest = find_latest_available_year(candidate, max_back=6)
        end = latest
        ...
        yrs = cached_years()
        if not yrs:
            raise RuntimeError("No cached CSV found in cache_life_csv. Set UPDATE_CACHE")
