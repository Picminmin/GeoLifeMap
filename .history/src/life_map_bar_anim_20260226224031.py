...
# ---- 大陸ごとに棒の色を統一 ----
CONT_COL = {
    "Asia": "rgba(31,119,180,0.85)",
    "Europe": "rgba(255,127,14,0.85)",
    "Africa": "rgba(44,160,44,0.85)",
    "North America": "rgba(214,39,40,0.85)",
    "South America": "rgba(148,103,189,0.85)",
    "Oceania": "rgba(140,86,75,0.85)",
    "Other": "rgba(127,127,127,0.75)",
}
    ...
    # ---- 大陸ごとの「行背景」(半透明の長方形) ----
    CONT_BG = {
        "Asia": "rgba(31,119,180,0.18)",
        "Europe": "rgba(255,127,14,0.18)",
        "Africa": "rgba(44,160,44,0.18)",
        "North America": "rgba(214,39,40,0.18)",
        "South America": "rgba(148,103,189,0.18)",
        "Oceania": "rgba(140,86,75,0.18)",
        "Other": "rgba(127,127,127,0.14)",
    }

    # 大陸ごとの背景ブロック
    N = len(y)
    row_shapes = []
    for i, c in enumerate(cont_seq):
        # 上から i行目を塗る(paper座標で0..1)
        y0 = 1 - (i + 1) / N
        y1 = 1 - i / N
        row_shapes.append(dict(
            ...
            x0=0, x1=1,
            y0=y0, y1=y1,
            fillcolor=CONT_BG.get(c, CONT_BG["Other"]),
            line=dict(width=0),
            layer="below"
        ))

    # 既存のshapesがあるなら足し合わせ
    fig.update_layout(shapes=(fig.layout.shapes or ()) + tuple(row_shapes))
