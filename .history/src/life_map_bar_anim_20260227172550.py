...
def detect_latest_year(timeout: int = 30) -> int:
    # 1国だけ見て、TimeDimensionValue の最大を拾う（Dim1=SEX_BTSX優先）
    probe = "JPN"  # USAでもOK
    params = {
        "$filter": f"SpatialDim eq '{probe}'",
        "$select": "TimeDimensionValue,Dim1",
        "$top": "5000",
    }
    js = requests.get(api_url, params=params, timeout=timeout).json()
    vals = js.get("value", [])
    if not vals:
        # 取れない場合は「今年-1」などにフォールバック（保険）
        return time.gmtime().tm_year - 1

    years = []
    for v in vals:
        if v.get("Dim1") == "SEX_BTSX":
            y = v.get("TimeDimensionValue")
    ...
    for v in vals:
        ...
        if y and str(y).isdigit():
            years.append(int(y))
    return max(years) if years else (time.gmtime().tm_year - 1)
...
def ensure_cache(start_year: int, end_year: int) -> None:
    ...
    for y in range(start_year, end_year + 1):
        ...
        df = fetch_year_df(y)
        print(f"[CACHE] year={y} rows={len(df)}")
# =========================
# 設定
# =========================
...
def main() -> None:
    ...
    latest = detect_latest_year()
    start = START_YEAR
    end = latest  # ← 最新年へ

    print(f"[INFO] years: {start}..{end}")
