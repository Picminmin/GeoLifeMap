...
def render_frame(df_year, df_prev, year, out_png, alpha=5, top_n=40) -> None:
    ...
    print("[DBG] year", year, "df_year rows", len(df_year), "cols", list(df_year.columns))
    print("[DBG] top head\n", df_year.head(3))
    print("[DBG] continent unique", df_year.get("continent", pd.Series(dtype=str)).dropna().unique()[:10])
...
[DBG] year 2000 df_year rows 185 cols ['iso3', 'life', 'continent', 'rank']
[DBG] top head
   iso3       life continent  rank
0  JPN  81.530179      Asia     1
1  CHE  79.714700    Europe     2
2  AUS  79.698319   Oceania     3
[DBG] continent unique ['Asia' 'Europe' 'Oceania' 'North America' 'Other' 'South America'
 'Africa']と表示されました。また、life_map_bar_anim.pyも最新版を添付しております。
