...
def render_frame(df_year, df_prev, year, out_png, alpha=5, top_n=40) -> None:
    ...
    # --- subtitle: continent counts in Top N ---
    ...
    # --- Left: choropleth ---
    fig.update_layout(margin=dict(l=110, r=40, t=60, b=40))
    ...
    # --- Right: bar chart (continent-colored) with legend ---
    # 大陸順を固定し、TopNに存在する大陸だけ描く
    ...
    # --- Bar visibility tuning: make bars look thicker ---
    fig.update_layout(
        barmode="overlay",
        ...
        legend=dict(
            title="Continent",
            x=0.985, y=0.02,           # 地図上の空きへ
            x=0.985, y=0.10,           # 地図上の空きへ
            xanchor="right",
            yanchor="bottom",
            bgcolor="rgba(255,255,255,0.80)",
            bordercolor="rgba(0,0,0,0.15)",
            borderwidth=1,
            font=dict(size=20),        # 大陸名を大きく
            title_font=dict(size=24),  # 凡例タイトルも大きく
            itemsizing="constant",
        )
    )
    ...
    # --- (optional) moved computation remains available ---
    # ここでは描画に使っていないが、将来「α以上変動した国だけ強調」などに使える
    moved = set()
    if df_prev is not None and not df_prev.empty and "rank" in df_prev.columns and "rank" in df_bar.columns:
    ...
    # --- Custom legend panel (2 rows x 3 cols) in bottom-left blank space ---
    # 表示したい大陸（Other は必要なら入れる。6枠ならOtherを外すのが綺麗）
    legend_items = [
        ...
        # ("Other", CONT_COL["Other"]),  # 7個目が必要なら別途レイアウト変更
    ]

    # 置き場所（paper座標: 0..1）
    # 左下の空白を想定して、適当にこのあたりからスタート
    panel_x0 = 0.06   # 左端
    panel_y0 = 0.11   # 下端
    cols = 3
    rows = 2
    ...
    cell_w = 0.12     # 1セルの幅
    cell_h = 0.045    # 1セルの高さ
    swatch_w = 0.015  # 色四角の幅
    swatch_h = 0.020  # 色四角の高さ
    ...
    fig.add_shape(
        ...
        x0=panel_x0 - 0.01, y0=panel_y0 - 0.01,
        x1=panel_x0 + panel_w + 0.01, y1=panel_y0 + panel_h + 0.01,
    ...
    for i, (name, col) in enumerate(legend_items):
        ...
        y_top = panel_y0 + (rows - 1 - r) * cell_h  # 上の行が先
