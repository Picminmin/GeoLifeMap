...
def render_frame(df_year, df_prev, year, out_png, alpha=5, top_n=40) -> None:
    ...
    if world_avg is not None: # ※ world_avg が計算済みであること（float）
        ...
        # 右列（棒）の xaxis domain（paper座標）を取る
        dom = fig.layout.xaxis.domain
        xL, xR = float(dom[0]), float(dom[1])
        
        # 画像出力サイズ（write_image の width と合わせる）
        FIG_W_PX = 1920
        ...
        # monospaceでの「1文字のだいたいの幅(px)」を近似（経験則：0.60 * font_size）
        # ここは微調整OK（0.55〜0.65くらいで好みが出る）
        mono_k = 0.60
        
        # label が占める幅(px) を近似
        label_px = int(len(label_text) * (label_font_size * mono_k)) + 18  # +少し余白
        
        # label 分の幅を paper 座標へ（右棒domain幅に対して）
        bar_domain_w = (xR - xL)
        label_frac = (label_px / FIG_W_PX)
        
        # 破線を「labelの左で止める」＝破線 + label を横並びにする
        # 右端ギリギリを避けつつ、label分だけ引く
        x0_line = xL + pad_lr
        x1_line = xR - pad_lr - label_frac
        # 念のため、短くなりすぎたら最低幅を確保
        x1_line = max(x1_line, x0_line + 0.10)
        
        # 破線のy位置（paper座標、下マージン領域）
        y_line = -0.140
        ...
        # --- 破線の「表示幅(px)」から、~~~ の文字数を逆算する（2行分） ---
        line_px = int((x1_line - x0_line) * FIG_W_PX)
        wavy_font_size = 18  # ← ここを上げると “~~” が見やすくなる
        char_px = wavy_font_size * mono_k
        n_tilde = max(8, int(line_px / char_px))  # 最低8文字は確保
        
        wavy = "<span style='font-family:monospace'>[" + ("~" * n_tilde) + "]</span>"
        
        # 破線と同じ長さ感で 2行の ~~~ を左寄せで置く（x0_line を基準に）
        fig.add_annotation(
            xref="paper", yref="paper",
            x=x0_line, y=-0.075,
            text=wavy,
            ...
            font=dict(size=wavy_font_size, color="rgba(0,0,0,0.60)"),
        )
        fig.add_annotation(
            xref="paper", yref="paper",
            x=x0_line, y=-0.105,
        ...
        # --- 右列（棒）の xaxis domain を取る（choropleth+xy では bar 側が xaxis）---
        dom = fig.layout.xaxis.domain  # 例: [0.72, 1.0]
        xL, xR = float(dom[0]), float(dom[1])

        # 文字列（見た目用）
        wavy = "<span style='font-family:monospace'>[~~~~~~~~~~~~~~~~~~~~~~~~~~~~]</span>"

        # 下に2行「~~~~」を置く（右の棒の下だけ）
        fig.add_annotation(
            xref="paper", yref="paper",
            x=(xL + xR) / 2, y=-0.075,   # 負値で下マージン領域へ
            text=wavy,
            showarrow=False,
            xanchor="center", yanchor="top",
            font=dict(size=14, color="rgba(0,0,0,0.55)"),
        )
        fig.add_annotation(
            xref="paper", yref="paper",
            x=(xL + xR) / 2, y=-0.105,
        ...
        fig.add_shape(
            type="line",
            xref="paper", yref="paper",
            x0=xL + 0.01, x1=xR - 0.01,
            y0=y_line, y1=y_line,
            line=dict(color="rgba(0,0,0,0.85)", width=2, dash="dash"),
            layer="above",
        )

        # 破線の右端に world avg 表記（例：world avg: 67.0）
        fig.add_annotation(
            xref="paper", yref="paper",
            x=xR - 0.01, y=y_line,
            text=f"<b>world avg:</b> {world_avg:.1f}",
            showarrow=False,
