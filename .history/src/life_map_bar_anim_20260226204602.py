...
# 1フレーム描画（左：地図 / 右：棒）
# =========================
def render_frame(df_year: pd.DataFrame, year: int, out_png: Path) -> None:
    if df_year.empty:
...
def render_frame(df_year, df_prev, year, out_png, alpha=5, top_n=40):
    # 右：表示する集合（その年の上位N）
    df_bar = df_year.head(top_n).copy()  # df_yearは既にlife降順・rank付与済み
    ...
    # （中略：左の地図traceはあなたの現状のままでOK）

    # 右：棒trace（marker.colorを指定）
    fig.add_trace(
        go.Bar(
            x=x, y=y,
            orientation="h",
    ...
    fig.update_xaxes(row=1, col=2, title_text="Years", range=[RANGE_COLOR[0], RANGE_COLOR[1]])
